...
public class DocAllianceDialog extends BaseMenuDialog {
    ...
    public DocAllianceDialog(Context context) {
        super(context);
    ...
    private void initViews() {
        ...
        mListView4.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                setDictItemClickListener(mMenuData1, mMenuData2, mMenuData3, mMenuData4);//选中点击的子菜单，去设置titleName
            }
        });
    ...
    private void getAlliance() {
        mMenuList1.clear();
        RequestParams params = new RequestParams(BaseConst.DEAULT_URL + "/mobile/clinic/getSiteUnionList?userId=" + mUserId);
            ...
            public void onSuccess(String result) {
                ...
                if (resultCode.equals("200")) {
                    ...
                    for (int a = 0; a < dataArray.size(); a++) {
                        String itemCode = dataArray.getJSONObject(a).get("id").toString();
                        String itemName = dataArray.getJSONObject(a).get("name").toString();
    ...
    private void getHospital(String itemCode) {
        RequestParams params = new RequestParams(BaseConst.DEAULT_URL + "/mobile/clinic/getSiteListByUnionId?id=" + itemCode);
        //params.setSslSocketFactory(); // 设置ssl
        params.addHeader("authorization", getTokenFromLocal());
        Callback.Cancelable cancelable;
        cancelable = x.http().get(params, new Callback.CommonCallback<String>() {
            @Override
            public void onSuccess(String result) {
                Log.i("test", "getAllianceHospital:" + result);
                com.alibaba.fastjson.JSONObject j = (com.alibaba.fastjson.JSONObject) com.alibaba.fastjson.JSONObject.parse(result);
                String resultCode = j.getString("resultCode");
                String resultMsg = j.getString("resultMsg");
                if (resultCode.equals("200")) {
                    ...
                    mMenuList2.clear();
                    for (int a = 0; a < dataArray.size(); a++) {
                        String siteid = dataArray.getJSONObject(a).get("site_id").toString();
