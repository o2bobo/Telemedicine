...
public class EoAdapter extends Adapter<ViewHolder> {

    private static final int TYPE_ITEM = 0;
    private static final int TYPE_FOOTER = 1;
    private static final int TYPE_FINISH = 2;
    private Context context;
    private ArrayList<EoItem> data;

    public DisplayImageOptions mOptions;
    public ImageLoader mImageLoader = ImageLoader.getInstance();


    public EoAdapter(Context context, ArrayList<EoItem> data) {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        if (holder instanceof CommenViewHolder) {
            ...
            String[] dateTime = data.get(position).startTime.split("\\s+");
            if (position == 0) {

                ((CommenViewHolder) holder).date.setText(dateTime[0]);
            } else {
                String[] temp = data.get(position).startTime.split("\\s+");
            ...
            ((CommenViewHolder) holder).msg.setText(data.get(position).operationName);
